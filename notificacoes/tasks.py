...
def enviar_notificacoes_boleto_async(self, parcela_id, tipo_template='BOLETO_CRIADO', anexar_pdf=True):
    """
    Tarefa assíncrona para envio de notificações de boleto (email + SMS + WhatsApp).
    Chamada após a geração do boleto para não bloquear a resposta ao usuário.

    Args:
        parcela_id: PK da Parcela
        tipo_template: Código do template (padrão: BOLETO_CRIADO)
        anexar_pdf: Se True, tenta anexar o PDF ao e-mail
    """
    try:
        from financeiro.models import Parcela as _Parcela
        from .boleto_notificacao import BoletoNotificacaoService

        parcela = _Parcela.objects.select_related(
            'contrato', 'contrato__comprador', 'contrato__imovel', 'contrato__imovel__imobiliaria'
        ).get(pk=parcela_id)

        service = BoletoNotificacaoService()
        resultado = service._notificar(parcela, tipo_template, anexar_pdf=anexar_pdf)

        email_ok = resultado.get('sucesso', False)
        sms_ok = resultado.get('sms', {}).get('sucesso', False) if resultado.get('sms') else None
        wpp_ok = resultado.get('whatsapp', {}).get('sucesso', False) if resultado.get('whatsapp') else None

        logger.info(
            "Notificações boleto parcela %s: email=%s, sms=%s, whatsapp=%s",
            parcela_id, email_ok, sms_ok, wpp_ok,
        )
        return {'email': email_ok, 'sms': sms_ok, 'whatsapp': wpp_ok}

    except _Parcela.DoesNotExist:
        logger.error("Parcela %s não encontrada para envio de notificações", parcela_id)
        return False
    except Exception as e:
        logger.exception("Erro ao enviar notificações de boleto para parcela %s: %s", parcela_id, e)
        try:
